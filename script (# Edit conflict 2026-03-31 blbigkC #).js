...
function getSeriesPeriodLabel(series) {
  const monthKeys = Array.isArray(series && series.monthKeys) ? series.monthKeys : [];
  if (monthKeys.length === 0) {
    return getFinanceChartScopeLabel();
  }
  return getFinanceChartScopeLabel(monthKeys.length, monthKeys[monthKeys.length - 1]);
}
...
function drawFinanceSeriesCanvas(canvas, series, options = {}) {
  ...
  const periodLabel = options.periodLabel || getSeriesPeriodLabel(series);
