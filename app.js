...
const api = {
    ...
    async sendDirectMessage(threadId, text) {
        const result = await apiRequest(`/api/directs/${encodeURIComponent(threadId)}/messages`, {
            method: "POST",
            body: { text }
        });
    ...
    async markDirectRead(threadId) {
        await apiRequest(`/api/directs/${encodeURIComponent(threadId)}/read`, {
            method: "POST"
        });
        return true;
    },
                    ...
                    ${
                        activeThread && otherUser
                            ? `
                                ...
                                <form class="comment-form direct-form" id="directForm" data-thread-id="${escapeHtml(activeThread.id)}">
                                    <input name="message" type="text" maxlength="1200" placeholder="Escreva uma mensagem" required>
                                    <button class="icon-button send-button" type="submit" aria-label="Enviar mensagem">
                                        <span class="button-symbol" aria-hidden="true">${renderIcon("send")}</span>
                                    </button>
                                </form>
                            `
                            : `
...
function refreshComposerSubmitState(form) {
    ...
    if (footnote) {
        footnote.textContent = hasTextContent ? "Pronto para publicar." : "Escreva o titulo ou o texto.";
    }
}
                ...
                <div class="composer-form-wrap">
                    ...
                    <form class="composer-form composer-form--discussion" id="composerForm">
                        ...
                        <div class="composer-footer composer-footer--discussion">
                            <span class="composer-footnote">${hasTextContent ? "Pronto para publicar." : "Escreva o titulo ou o texto."}</span>
                            <button class="primary-button" type="submit" ${canPublish ? "" : "disabled"}>
                                ${renderButtonContent("compose", "Publicar")}
                            </button>
                        </div>
                    </form>
                </div>
