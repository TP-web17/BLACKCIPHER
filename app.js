...
function createBusyOverlayState(active = false, title = "Carregando", message = "Aguarde um instante.") {
    return {
        active: Boolean(active),
        title: sanitizeText(title, 64) || "Carregando",
        message: sanitizeText(message, 140) || "Aguarde um instante."
    };
}
...
const state = {
    ...
    ui: {
        ...
        busyOverlay: createBusyOverlayState(
            true,
            "Entrando na conta",
            "Reconectando sua sessao e carregando a rede."
        )
    }
};
...
function loadDb() {
    try {
        ...
        // Ignora quota/storage bloqueado: o backend agora e a fonte principal.
    }
...
function cleanupLegacyStorage() {
    [...LEGACY_KEYS, DB_KEY].forEach((key) => {
        try {
...
function cleanupDatabase() {
    ...
    state.db.posts.forEach((post) => {
        ...
        validPosts.push({
            ...
            contentMode: normalizeComposerContentMode(post.contentMode || (sanitizedImage ? "media" : "text")),
            distribution: normalizePostDistribution(post.distribution || (post.postKind === "discussion" ? "discussion" : "both")),
    ...
    state.db.boostsByUser = cleanInteractionMap(state.db.boostsByUser, validUserIds, validPostIds);
    ...
    state.db.alertsSeenAtByUser = cleanTimestampMap(state.db.alertsSeenAtByUser, validUserIds);
    state.db.viewHistoryByUser = cleanViewHistoryMap(state.db.viewHistoryByUser, validUserIds, validPostIds);
    state.db.directThreads = cleanDirectThreads(state.db.directThreads, validUserIds);
...
function persistDb() {
    try {
        ...
        // O app nao persiste mais snapshots grandes no navegador.
    }
}
...
function normalizeClientDb(rawDb) {
    ...
    return {
        ...
        posts: Array.isArray(rawDb.posts) ? rawDb.posts : [],
        likesByUser: isPlainObject(rawDb.likesByUser) ? rawDb.likesByUser : {},
        boostsByUser: isPlainObject(rawDb.boostsByUser) ? rawDb.boostsByUser : {},
        savesByUser: isPlainObject(rawDb.savesByUser) ? rawDb.savesByUser : {},
        followsByUser: isPlainObject(rawDb.followsByUser) ? rawDb.followsByUser : {},
        commentsByPost: isPlainObject(rawDb.commentsByPost) ? rawDb.commentsByPost : {},
        activitiesByUser: isPlainObject(rawDb.activitiesByUser) ? rawDb.activitiesByUser : {},
        alertsSeenAtByUser: isPlainObject(rawDb.alertsSeenAtByUser) ? rawDb.alertsSeenAtByUser : {},
        viewHistoryByUser: isPlainObject(rawDb.viewHistoryByUser) ? rawDb.viewHistoryByUser : {},
        directThreads: Array.isArray(rawDb.directThreads) ? rawDb.directThreads : [],
        recentAccountIds: Array.isArray(rawDb.recentAccountIds) ? rawDb.recentAccountIds : []
    };
}
...
async function bootstrapSession() {
    try {
        ...
        state.ui.bootstrappingSession = false;
        clearBusyOverlay();
        renderAll();
    }
}
...
function bindEvents() {
    ...
    loginForm.addEventListener("submit", async (event) => {
        ...
        state.ui.authPendingMode = "login";
        renderAuthScreen();
        setBusyOverlay("Entrando na conta", "Validando sua senha e abrindo o seu perfil.");

        try {
            await api.loginWithHandle(handle, password);
            loginForm.reset();
            state.ui.query = "";
            renderAll();
    ...
    registerForm.addEventListener("submit", async (event) => {
        ...
        state.ui.authPendingMode = "register";
        renderAuthScreen();
        setBusyOverlay("Criando sua conta", "Preparando o perfil inicial e conectando voce na rede.");
    ...
    document.addEventListener("click", async (event) => {
        ...
        if (tabButton) {
            ...
            if (group === "alertsPreview") {
                state.ui.alertsPreviewTab = value === "history" ? "history" : "unread";
                if (currentUser) {
                    renderTopbarAlertsPreview(currentUser);
                }
                return;
        ...
        try {
            if (action === "logout") {
                ...
                state.ui.profileConnectionsView = "";
                state.ui.alertsPreviewOpen = false;
                state.ui.alertsPreviewTab = "unread";
                state.ui.directThreadId = null;
                state.ui.orderSnapshots = {};
                state.ui.authPendingMode = "";
                state.ui.publishingPost = false;
                state.ui.sendingDirect = false;
                state.ui.busyOverlay = createBusyOverlayState();
                renderAll();
                showToast("Voce saiu.");
                return;
            }
            ...
            if (action === "composer-set-mode") {
                const mode = actionButton.dataset.mode || "media";
                const draft = state.ui.composerDraft;

                if (mode === "discussion") {
                    draft.postKind = "discussion";
                    draft.contentMode = "text";
                    draft.distribution = "discussion";
                    draft.category = "Editorial";
                    draft.imageData = "";
                    draft.fileName = "";
                    draft.presentation = {
                        ...normalizePostPresentation(draft.presentation),
                        sticker: createDefaultSticker()
                    };
                } else {
                    draft.postKind = "art";
                    draft.contentMode = mode === "text" ? "text" : "media";
                    draft.distribution = normalizePostDistribution(draft.distribution === "discussion" ? "both" : draft.distribution);
                    draft.category = draft.category === "Editorial" ? "Ilustracao" : draft.category || "Ilustracao";
                }
            ...
            if (action === "composer-set-distribution") {
                const draft = state.ui.composerDraft;

                if (normalizePostKind(draft.postKind) !== "discussion") {
                    draft.distribution = normalizePostDistribution(actionButton.dataset.distribution || draft.distribution);
                    renderComposerModal();
                }
            ...
            if (action === "toggle-alerts-preview") {
                const nextOpenState = !state.ui.alertsPreviewOpen;
                state.ui.alertsPreviewOpen = nextOpenState;
                state.ui.profileMenuOpen = false;
            ...
            if (action === "mark-alerts-read") {
                const user = requireCurrentUser();
                const optimisticSeenAt = markAlertsAsSeen(user.id);
                renderTopbarAlertsPreview(user);
                syncTopbarAlertsBadge(user);
                renderView();
                try {
                    const serverSeenAt = await api.markAlertsAsSeen();
                    state.db.alertsSeenAtByUser[user.id] = Math.max(optimisticSeenAt, serverSeenAt);
                } catch (error) {
                    throw error;
                }
                return;
            ...
            if (action === "open-direct-with-user") {
                const targetUserId = actionButton.dataset.userId || "";
            ...
            if (action === "toggle-user-ban") {
                ...
                const isBanned = actionButton.dataset.isBanned === "true";
                await api.setUserBanState(userId, !isBanned);
                renderFrame();
                return;
            }
            ...
            if (action === "delete-user-account") {
                ...
                const confirmed = window.confirm("Excluir esse perfil por completo? Essa acao remove posts, comentarios e directs dessa conta.");
                ...
                if (state.ui.profileUserId === userId) {
                    state.ui.view = "moderation";
                    state.ui.profileUserId = getCurrentUser()?.id || null;
                }

                if (state.ui.directThreadId) {
                    const stillExists = getDirectThreadsForUser(getCurrentUser()?.id || "").some((thread) => thread.id === state.ui.directThreadId);
                    if (!stillExists) {
                        state.ui.directThreadId = null;
                    }
                }
            ...
            if (action === "toggle-profile-connections") {
                ...
                if (state.ui.view !== "profile") {
                    state.ui.view = "profile";
                    state.ui.profileUserId = state.ui.profileUserId || user.id;
                    state.ui.profileMenuOpen = false;
                    state.ui.alertsPreviewOpen = false;
                }
            ...
            if (action === "set-site-theme") {
                state.prefs.theme = normalizeSiteTheme(actionButton.dataset.themeValue || state.prefs.theme);
                applyTheme(state.prefs.theme);
                persistPrefs();
                renderTopUserPill(requireCurrentUser());
                return;
            }

            if (action === "set-site-skin") {
                state.prefs.skin = normalizeSiteSkin(actionButton.dataset.skinValue || state.prefs.skin);
                applyTheme(state.prefs.theme);
                persistPrefs();
                renderTopUserPill(requireCurrentUser());
                return;
            ...
            if (action === "toggle-like") {
                ...
                const likedBefore = getLikedIds(currentUser.id).includes(postId);
                const optimisticLiked = !likedBefore;
                setLikeState(currentUser.id, postId, optimisticLiked);
                ...
                try {
                    const serverLiked = await api.toggleLike(postId);

                    if (serverLiked !== optimisticLiked) {
                        setLikeState(currentUser.id, postId, serverLiked);
                    ...
                    setLikeState(currentUser.id, postId, likedBefore);
            ...
            if (action === "toggle-boost") {
                const postId = actionButton.dataset.postId || "";
                const currentUser = requireCurrentUser();
                const boostedBefore = getBoostedIds(currentUser.id).includes(postId);
                const optimisticBoosted = !boostedBefore;
                setBoostState(currentUser.id, postId, optimisticBoosted);
                renderFrame();
                renderPostModal();

                try {
                    const serverBoosted = await api.toggleBoost(postId);

                    if (serverBoosted !== optimisticBoosted) {
                        setBoostState(currentUser.id, postId, serverBoosted);
                        renderFrame();
                        renderPostModal();
                    }
                } catch (error) {
                    setBoostState(currentUser.id, postId, boostedBefore);
                    renderFrame();
                    renderPostModal();
                    throw error;
                }
            ...
            if (action === "toggle-save") {
                const postId = actionButton.dataset.postId || "";
                const currentUser = requireCurrentUser();
                const savedBefore = getSavedIds(currentUser.id).includes(postId);
                const optimisticSaved = !savedBefore;
                setSaveState(currentUser.id, postId, optimisticSaved);
                renderFrame();
                renderPostModal();

                try {
                    const serverSaved = await api.toggleSave(postId);

                    if (serverSaved !== optimisticSaved) {
                        setSaveState(currentUser.id, postId, serverSaved);
                        renderFrame();
                        renderPostModal();
                    }
                } catch (error) {
                    setSaveState(currentUser.id, postId, savedBefore);
                    renderFrame();
                    renderPostModal();
                    throw error;
                }
            ...
            if (action === "toggle-follow") {
                const targetUserId = actionButton.dataset.userId || "";
                const currentUser = requireCurrentUser();
                const followingBefore = getFollowingIds(currentUser.id).includes(targetUserId);
                const optimisticFollowing = !followingBefore;
                setFollowState(currentUser.id, targetUserId, optimisticFollowing);
                renderFrame();
                renderPostModal();

                try {
                    const serverFollowing = await api.toggleFollow(targetUserId);

                    if (serverFollowing !== optimisticFollowing) {
                        setFollowState(currentUser.id, targetUserId, serverFollowing);
                        renderFrame();
                        renderPostModal();
                    }
                } catch (error) {
                    setFollowState(currentUser.id, targetUserId, followingBefore);
                    renderFrame();
                    renderPostModal();
                    throw error;
                }
            ...
            if (action === "delete-comment") {
                ...
                removeCommentsByIds(postId, deletedIds);
    ...
    document.addEventListener("submit", async (event) => {
        ...
        const directForm = event.target.closest("#directForm");

        if (directForm) {
            event.preventDefault();

            if (state.ui.sendingDirect) {
                return;
            }

            const formData = new FormData(directForm);
            const text = String(formData.get("message") || "");
            const threadId = directForm.dataset.threadId || state.ui.directThreadId || "";
            const currentUser = requireCurrentUser();
            state.ui.sendingDirect = true;
            syncDirectFormState();

            try {
                const message = await api.sendDirectMessage(threadId, text);
                if (message) {
                    appendDirectMessage(threadId, {
                        ...message,
                        readBy: Array.isArray(message.readBy) ? message.readBy : [currentUser.id]
                    });
                }
                directForm.reset();
                renderFrame();
        ...
        if (commentForm) {
            ...
            try {
                const nextComment = await api.addComment(postId, text, replyToCommentId);
                if (nextComment) {
        ...
        if (composerForm) {
            ...
            if (state.ui.publishingPost) {
                return;
            }

            syncComposerDraftFromLiveForm();
            state.ui.publishingPost = true;
            renderComposerModal();
            setBusyOverlay("Publicando seu post", "Enviando a arte e atualizando o feed sem duplicar a postagem.");
                ...
                state.ui.view =
                    createdPost.distribution === "discussion"
                        ...
                        : createdPost.distribution === "feed"
                          ? "following"
                          : "home";
                state.ui.postModalId = createdPost.distribution === "discussion" ? null : createdPost.id;
                if (createdPost.distribution === "discussion") {
                    state.ui.viewTabs.discussions = "recent";
                    state.ui.query = "";
                }
                state.ui.composerOpen = false;
                state.ui.composerDraft = createEmptyDraft();
        ...
        if (profileForm) {
            ...
            try {
                if (currentPassword || newPassword || confirmPassword) {
                    if (!currentPassword) {
                        throw new Error("Digite sua senha atual para trocar a senha.");
                    }
                    ...
                    if (newPassword !== confirmPassword) {
                        throw new Error("A confirmacao da nova senha nao bate.");
                    }
                ...
                state.ui.profileEditorOpen = false;
                renderAll();
                showToast(currentPassword || newPassword || confirmPassword ? "Perfil e senha atualizados." : "Perfil atualizado.");
            } catch (error) {
    ...
    document.addEventListener("input", (event) => {
        ...
        if (composerForm) {
            ...
            refreshComposerSubmitState(composerForm);
    ...
    document.addEventListener("change", async (event) => {
        ...
        const composerForm = event.target.closest("#composerForm");
        const composerKindSelect = event.target.closest('select[name="postKind"]');
        const composerContentModeSelect = event.target.closest('select[name="contentMode"]');
        ...
        if (composerForm) {
            ...
            if (composerKindSelect || composerContentModeSelect) {
                renderComposerModal();
                return;
            }
        ...
        if (fileInput && fileInput.files && fileInput.files[0]) {
            try {
                ...
                if (normalizePostKind(state.ui.composerDraft.postKind) === "discussion") {
                    state.ui.composerDraft.contentMode = "media";
                }
                state.ui.uploadingImage = false;
                renderComposerModal();
...
const api = {
    async registerUser(payload) {
        ...
        return getUserById(result.db?.sessionUserId || state.db.sessionUserId);
    },

    async loginWithHandle(rawHandle, rawPassword) {
    ...
    async createPost(payload) {
        ...
        return state.db.posts.find((post) => post.id === result.postId) || state.db.posts[0] || null;
    },
    ...
    async toggleLike(postId) {
        const result = await apiRequest(`/api/posts/${encodeURIComponent(postId)}/like`, {
            method: "POST"
        });
    ...
    async toggleBoost(postId) {
        const result = await apiRequest(`/api/posts/${encodeURIComponent(postId)}/boost`, {
            method: "POST"
        });
        return Boolean(result.boosted);
    },

    async toggleSave(postId) {
        const result = await apiRequest(`/api/posts/${encodeURIComponent(postId)}/save`, {
            method: "POST"
        });
    ...
    async toggleFollow(targetUserId) {
        const result = await apiRequest(`/api/follows/${encodeURIComponent(targetUserId)}`, {
            method: "POST"
        });
    ...
    async addComment(postId, rawText, replyToCommentId = "") {
        const result = await apiRequest(`/api/posts/${encodeURIComponent(postId)}/comments`, {
            method: "POST",
            body: {
                text: rawText,
                replyToCommentId
    ...
    async deleteComment(postId, commentId) {
        const result = await apiRequest(`/api/posts/${encodeURIComponent(postId)}/comments/${encodeURIComponent(commentId)}`, {
            method: "DELETE"
        });
        return Array.isArray(result.deletedCommentIds) ? result.deletedCommentIds : [commentId];
    },

    async markAlertsAsSeen() {
        const result = await apiRequest("/api/alerts/seen", {
            method: "POST"
        });
        return Math.max(0, Number(result.seenAt) || 0);
    },

    async recordPostView(postId, options = {}) {
        await apiRequest(`/api/posts/${encodeURIComponent(postId)}/view`, {
            method: "POST",
            body: options
        });
    },

    async startDirectThread(targetUserId) {
        const normalizedTargetUserId = sanitizeText(String(targetUserId || ""), 120);

        if (!normalizedTargetUserId) {
            throw new Error("Escolha um perfil antes de abrir o direct.");
        }

        const result = await apiRequest("/api/directs/start", {
            method: "POST",
            body: { targetUserId: normalizedTargetUserId }
        });
        return result.threadId || null;
    },

    async sendDirectMessage(threadId, text) {
        const normalizedThreadId = sanitizeText(String(threadId || ""), 120);
        const normalizedText = sanitizeText(String(text || ""), 1200);

        if (!normalizedThreadId) {
            throw new Error("Abra uma conversa antes de enviar.");
        }

        if (!normalizedText) {
            throw new Error("Escreva algo antes de enviar.");
        }

        const result = await apiRequest(`/api/directs/${encodeURIComponent(normalizedThreadId)}/messages`, {
            method: "POST",
    ...
    async markDirectRead(threadId) {
        ...
        await apiRequest(`/api/directs/${encodeURIComponent(normalizedThreadId)}/read`, {
            method: "POST"
        });
        return true;
    },

    async setUserBanState(userId, isBanned) {
...
function setBusyOverlay(title, message) {
    state.ui.busyOverlay = createBusyOverlayState(true, title, message);
    renderLoadingOverlay();
}
...
function renderLoadingOverlay() {
    if (!loadingOverlay || !loadingOverlayTitle || !loadingOverlayText) {
        return;
    }

    const overlayState = state.ui.busyOverlay || createBusyOverlayState();
    loadingOverlay.classList.toggle("is-hidden", !overlayState.active);
    loadingOverlay.classList.toggle("is-visible", overlayState.active);
    loadingOverlay.setAttribute("aria-hidden", overlayState.active ? "false" : "true");
    loadingOverlayTitle.textContent = overlayState.title;
    loadingOverlayText.textContent = overlayState.message;
    document.body.classList.toggle("is-busy", overlayState.active);
}

function syncFormDisabled(form, isDisabled) {
    if (!form) {
        return;
    }
...
function syncAuthSubmitState() {
    const isAuthLocked = state.ui.bootstrappingSession || Boolean(state.ui.authPendingMode);
    const loginButton = loginForm?.querySelector('button[type="submit"]');
    const registerButton = registerForm?.querySelector('button[type="submit"]');

    document.querySelectorAll("[data-auth-view]").forEach((button) => {
        button.disabled = isAuthLocked;
    });

    syncFormDisabled(loginForm, isAuthLocked);
    syncFormDisabled(registerForm, isAuthLocked);

    if (loginButton) {
        loginButton.textContent = state.ui.authPendingMode === "login" ? "Entrando..." : "Entrar";
    }

    if (registerButton) {
        registerButton.textContent = state.ui.authPendingMode === "register" ? "Criando..." : "Criar conta";
    }
}
...
function renderSavedAccounts() {
    savedAccounts.classList.add("is-empty");
...
function renderTopbarAlertsPreview(user) {
    ...
    const previewTab = state.ui.alertsPreviewTab === "history" ? "history" : "unread";
    const activities = (previewTab === "history" ? getReadActivities(user.id) : getUnreadActivities(user.id)).slice(0, 5);
        ...
        <div class="top-alerts-menu">
            <div class="top-alerts-head">
                <div class="card-copy">
                    ...
                    <span>${escapeHtml(previewTab === "history" ? "Historico recente da sua conta." : "O que chegou agora para voce.")}</span>
                </div>
                <div class="top-alerts-head-actions">
                    <button class="ghost-button ghost-button--compact ${previewTab === "unread" ? "is-active" : ""}" type="button" data-tab-group="alertsPreview" data-tab-value="unread">
                        Novas
                    </button>
                    <button class="ghost-button ghost-button--compact ${previewTab === "history" ? "is-active" : ""}" type="button" data-tab-group="alertsPreview" data-tab-value="history">
                        Historico
                    </button>
                    ${
                        getUnreadActivities(user.id).length
                            ? `
                                <button class="ghost-button ghost-button--compact" type="button" data-action="mark-alerts-read">
                                    Ler tudo
                                </button>
                            `
                            : ""
                    ...
                    activities.length
                        ...
                        : `
                            <div class="top-alert-empty">
                                <strong>${escapeHtml(previewTab === "history" ? "Sem historico por enquanto." : "Sem novidades por enquanto.")}</strong>
                                <span>${escapeHtml(previewTab === "history" ? "Quando voce receber alertas e marcar como lidos, eles ficam guardados aqui." : "Quando alguem curtir, comentar, seguir ou mandar direct, aparece aqui.")}</span>
                            </div>
                        `
                }
            </div>
...
function renderTopbarAlertsPreviewItem(entry) {
    ...
    const action = entry.type === "message" ? "open-direct-with-user" : post ? "open-post" : "open-profile";
...
function renderTopUserPill(user) {
    ...
    const siteTheme = normalizeSiteTheme(state.prefs.theme);
    const siteSkin = normalizeSiteSkin(state.prefs.skin);
    ...
    const latestPosts = getUserPosts(user.id).slice(0, 1);
                        ...
                        <div class="top-profile-menu">
                            <button class="top-profile-summary" type="button" data-action="open-own-profile">
                                ...
                                <div class="top-profile-summary-copy">
                                    ...
                                    ${user.statusNote ? `<small>${escapeHtml(user.statusNote)}</small>` : ""}
                                    <div class="top-profile-summary-flags">
                                        ${renderUserPrivacyPresencePills(user, user.id, { includeHiddenHint: true })}
                                    </div>
                                </div>
                            </button>
                            ...
                            <div class="top-profile-menu-actions">
                                ...
                                <button class="profile-menu-button" type="button" data-action="open-messages">
                                    <span class="button-symbol" aria-hidden="true">${renderIcon("message")}</span>
                                    <span>Direct</span>
                                </button>
                                ...
                                ${
                                    isModerator(user)
                                        ? `
                                            <button class="profile-menu-button" type="button" data-action="open-moderation">
                                                <span class="button-symbol" aria-hidden="true">${renderIcon("shield")}</span>
                                                <span>Moderacao</span>
                                            </button>
                                        `
                                        : ""
                            ...
                            <div class="top-profile-color-studio">
                                <div class="card-copy">
                                    ...
                                    <span>Cor, modo e skin.</span>
                                </div>
                                <div class="top-profile-mode-group">
                                    <span class="top-profile-mode-label">Modo</span>
                                    <div class="top-profile-mode-row">
                                        ${SITE_THEME_OPTIONS.map(
                                            (option) => `
                                                <button
                                                    class="top-profile-mode-chip ${siteTheme === option.id ? "is-active" : ""}"
                                                    type="button"
                                                    data-action="set-site-theme"
                                                    data-theme-value="${escapeHtml(option.id)}"
                                                >
                                ...
                                <div class="top-profile-mode-group">
                                    <span class="top-profile-mode-label">Skin</span>
                                    <div class="top-profile-mode-row">
                                        ${SITE_SKIN_OPTIONS.map(
                                            (option) => `
                                                <button
                                                    class="top-profile-mode-chip ${siteSkin === option.id ? "is-active" : ""}"
                                                    type="button"
                                                    data-action="set-site-skin"
                                                    data-skin-value="${escapeHtml(option.id)}"
                                                >
                                                    ${escapeHtml(option.label)}
                                                </button>
                                            `
                                ...
                                <div class="top-profile-mode-group">
                                    <span class="top-profile-mode-label">Cor</span>
                                </div>
...
function renderView() {
    ...
    const markup =
        state.ui.view === "discussions"
            ...
            : state.ui.view === "messages"
            ? renderMessagesView(currentUser)
            ...
            : state.ui.view === "saved"
              ...
              : state.ui.view === "activity"
                ...
                : state.ui.view === "moderation" && isModerator(currentUser)
                  ? renderModerationView(currentUser)
                  : state.ui.view === "profile"
...
function syncDirectFormState() {
    ...
    if (messageInput) {
        messageInput.disabled = state.ui.sendingDirect;
    }

    if (submitButton) {
        submitButton.disabled = state.ui.sendingDirect;
        submitButton.classList.toggle("is-loading", state.ui.sendingDirect);
        submitButton.setAttribute("aria-label", state.ui.sendingDirect ? "Enviando mensagem" : "Enviar mensagem");
        submitButton.innerHTML = renderDirectSubmitButton(state.ui.sendingDirect);
    }
}

function scrollActiveDirectThreadToEnd() {
    window.requestAnimationFrame(() => {
        const threadContainer = viewRoot.querySelector(".messages-thread");
...
function renderHomePhotoIntro(homeMode, posts) {
    ...
    return `
        <section class="photo-mode-head photo-mode-head--minimal">
            <div class="photo-mode-copy">
                <span class="section-kicker">Fotos</span>
                <div>
                    <h1>Colagem da rede</h1>
                    <p>${escapeHtml(`${formatMetricLabel(photoCount, "imagem", "imagens")} em uma leitura direta, continua e focada so na arte.`)}</p>
                </div>
            </div>
...
function renderHomeSpotlight(user, posts) {
    ...
    if (!spotlight) {
        return renderHero({
            ...
            text: "Uma leitura mais limpa do que esta puxando a rede agora.",
            stats: [
    ...
    return `
        <section class="view-hero view-hero--editorial view-hero--minimal-home">
            <div class="hero-main hero-main--minimal">
                <div class="hero-copy-shell">
                    <span class="section-kicker">Inicio</span>
                    <div>
                        <h1>Hoje no CONQUEST</h1>
                        <p>Uma primeira dobra mais direta, com menos ruido e foco no que esta chamando atencao agora.</p>
                    </div>
                    <div class="hero-actions hero-actions--compact">
                        <button class="primary-button" type="button" data-view="following">
                            ${renderButtonContent("open", "Abrir feed")}
                        </button>
                    </div>
                </div>
                <div class="hero-feature hero-feature--minimal">
                    ...
                    <div class="hero-feature-copy ${getPostFontClass(spotlight)}"${spotlightStyle}>
                        ...
                        <div>
                            ...
                            <p>${escapeHtml(truncateText(spotlight.caption || "Uma obra em destaque para puxar o ritmo do dia.", 112))}</p>
                        </div>
...
function renderDiscussionView(user) {
    ...
    const note = query
        ...
        : discussionTab === "recent"
          ? `${formatCompact.format(recentPosts.length)} discussoes em ordem cronologica.`
          : discussionTab === "following"
        ...
        <div class="view-stack view-stack--discussions">
            <section class="discussion-hero">
                <div class="discussion-hero-copy">
                    <span class="section-kicker">Discussoes</span>
                    <div>
                        <h1>${escapeHtml(query ? `Conversas sobre ${query}` : "Assuntos do momento")}</h1>
                        <p>Um espaco mais direto para abrir conversas, responder rapido e manter a leitura limpa.</p>
                    </div>
                </div>
                ...
                <div class="discussion-main">
                    <section class="tag-strip tag-strip--modes tag-strip--discussion">
                        <div class="strip-copy strip-copy--discussion">
                            <span class="section-kicker">Timeline</span>
                            <p>
                                <strong class="strip-title">${escapeHtml(query ? "Busca em discussoes" : "Conversa viva")}</strong>
                                <span>${escapeHtml(note)}</span>
                            </p>
                        </div>
                    ...
                    <section class="discussion-topic-panel">
                        <div class="discussion-topic-head">
                            <div class="card-copy">
                                ...
                                <span>Toque em uma hashtag para filtrar a conversa.</span>
                            </div>
                        </div>
...
function renderActivityView(user) {
    const unreadActivities = getUnreadActivities(user.id);
    const readActivities = getReadActivities(user.id);
    const activityMode = state.ui.viewTabs.activity === "history" ? "history" : "unread";
    const activities = activityMode === "history" ? readActivities : unreadActivities;
    const activityTitle =
        activityMode === "history"
            ? "Seu historico de alertas continua guardado aqui."
            : "Novidades que ainda merecem seu olhar.";
    const activityText =
        activityMode === "history"
    ...
    return renderSocialScreen({
        ...
        main: `
            ${renderViewTabsStrip({
                ...
                title: "Veja o que chegou agora e o que ja entrou no historico.",
                text: "Voce marca as visiveis como lidas sem apagar nada da base.",
                tabs: [
                    { value: "unread", label: "Novas", count: unreadActivities.length },
                    { value: "history", label: "Historico", count: readActivities.length }
                ]
            })}
            ${
                ...
                <aside class="messages-list">
                    ...
                    ${
                        threads.length
                            ? threads
                                  .map((thread) => {
                                      const peer = getThreadOtherUser(thread, user.id);
                                      const lastMessage = thread.messages[thread.messages.length - 1] || null;
                                      const unread = getUnreadDirectCountForThread(thread, user.id);

                                      return `
                                          <button class="message-thread-card ${activeThread?.id === thread.id ? "is-active" : ""}" type="button" data-action="open-thread" data-thread-id="${escapeHtml(thread.id)}">
                                              ${peer ? renderAvatar(peer, "mini-avatar") : `<span class="mini-avatar">C</span>`}
                                        ...
                                        activeThread.messages.length
                                            ...
                                            : `
                                                <div class="top-alert-empty">
                                                    ...
                                                    <span>Mande a primeira mensagem e comece o direct.</span>
                                                </div>
                                            `
                                    }
                                </div>
...
function renderModerationView(user) {
    const users = state.db.users.filter((profileUser) => profileUser.id !== user.id);
    const recentPosts = state.db.posts.slice().sort((a, b) => b.createdAt - a.createdAt).slice(0, 8);

    return `
        <div class="view-stack">
            ${renderHero({
                kicker: "Moderacao",
                title: "Painel inicial da equipe",
                text: "Uma base simples para revisar usuarios, bloquear contas e apagar conteudo sem poluir o restante do app.",
                stats: [
                    { value: formatCompact.format(users.length), label: "usuarios" },
                    { value: formatCompact.format(recentPosts.length), label: "posts recentes" },
                    { value: formatCompact.format(users.filter((profileUser) => profileUser.isBanned).length), label: "bloqueadas" }
                ]
            })}
            <section class="moderation-grid">
                <section class="section-shell">
                    ...
                    <div class="moderation-user-list">
                        ${users
                            .map(
                                (profileUser) => `
                                    <article class="moderation-user-card">
                                        <div class="comment-row">
                                            ${renderAvatar(profileUser, "mini-avatar")}
                                            <div class="card-copy">
                                                <strong>${escapeHtml(profileUser.name)}</strong>
                                                <span>@${escapeHtml(profileUser.handle)} · ${escapeHtml(profileUser.isBanned ? "Bloqueada" : "Ativa")}</span>
                                            </div>
                                        </div>
                                        <div class="moderation-user-actions">
                                            <button class="ghost-button ghost-button--compact" type="button" data-action="open-profile" data-user-id="${escapeHtml(profileUser.id)}">Perfil</button>
                                            <button class="ghost-button ghost-button--compact" type="button" data-action="toggle-user-ban" data-user-id="${escapeHtml(profileUser.id)}" data-is-banned="${profileUser.isBanned ? "true" : "false"}">
                                                ${escapeHtml(profileUser.isBanned ? "Desbanir" : "Banir")}
                                            </button>
                                            <button class="ghost-button ghost-button--compact ghost-button--danger" type="button" data-action="delete-user-account" data-user-id="${escapeHtml(profileUser.id)}">
                                                Excluir
                                            </button>
                                        </div>
                                    </article>
                ...
                <section class="section-shell">
                    <div class="section-head">
                        <div>
                            <span class="section-kicker">Conteudo</span>
                            <h2>Posts recentes</h2>
                        </div>
                    </div>
                    <div class="moderation-post-list">
                        ${recentPosts
                            .map((post) => {
                                const author = getUserById(post.authorId);
                                return `
                                    <article class="moderation-post-card">
                                        <div class="card-copy">
                                            <strong>${escapeHtml(post.title)}</strong>
                                            <span>${escapeHtml(author ? `@${author.handle}` : "autor")} · ${escapeHtml(timeAgo(post.createdAt))}</span>
                                        </div>
                                        <div class="moderation-user-actions">
                                            <button class="ghost-button ghost-button--compact" type="button" data-action="open-post" data-post-id="${escapeHtml(post.id)}">Abrir</button>
                                            <button class="ghost-button ghost-button--compact" type="button" data-action="delete-post" data-post-id="${escapeHtml(post.id)}">Apagar</button>
                                        </div>
                                    </article>
                                `;
                            })
...
function renderProfileView(profileUser, currentUser) {
    ...
    const canAccessContent = canViewerAccessUserContent(profileUser, viewerId);
    const posts = canAccessContent
        ? applyPostFilters(getUserPosts(profileUser.id).map((post) => toPostView(post, viewerId)).filter(Boolean))
        : [];
        ...
        main: `
            <section class="profile-hero" data-profile-theme="${escapeAttribute(profileTheme.id)}">
                ...
                <div class="profile-hero-content">
                                ...
                                isOwnProfile
                                    ...
                                    : `
                                        <div class="profile-action-row">
                                            ...
                                            >
                                                ${renderButtonContent("follow", isFollowed ? "Seguindo" : isProfilePrivate(profileUser) ? "Seguir para ver" : "Seguir")}
                                            </button>
                                            <button class="ghost-button" type="button" data-action="open-direct-with-user" data-user-id="${escapeHtml(profileUser.id)}">
                                                ${renderButtonContent("message", "Mensagem")}
                                            </button>
                                            ${
                                                isModerator(currentUser)
                                                    ? `
                                                        <button class="ghost-button ghost-button--danger" type="button" data-action="toggle-user-ban" data-user-id="${escapeHtml(profileUser.id)}" data-is-banned="${profileUser.isBanned ? "true" : "false"}">
                                                            ${renderButtonContent("delete", profileUser.isBanned ? "Desbanir" : "Banir")}
                                                        </button>
                                                        <button class="ghost-button ghost-button--danger" type="button" data-action="delete-user-account" data-user-id="${escapeHtml(profileUser.id)}">
                                                            ${renderButtonContent("delete", "Excluir perfil")}
                                                        </button>
                                                    `
                                                    : ""
                                            }
                    ...
                    ${profileUser.statusNote ? `<p class="profile-status-note">${escapeHtml(profileUser.statusNote)}</p>` : ""}
            ...
            ${
                !canAccessContent && !isOwnProfile
                    ? `
                        <section class="section-shell profile-private-lock">
                            <div class="section-head">
                                <div>
                                    <span class="section-kicker">Conta privada</span>
                                    <h2>Esse perfil protege os posts e colecoes.</h2>
                                    <p class="section-note">Siga @${escapeHtml(profileUser.handle)} para ver as obras, o destaque e o movimento completo desse espaco.</p>
                                </div>
                                <button
                                    class="follow-button ${isFollowed ? "is-active" : ""}"
                                    type="button"
                                    ...
                                    data-user-id="${escapeHtml(profileUser.id)}"
                                >
                                    ${renderButtonContent("follow", isFollowed ? "Seguindo" : "Seguir para ver")}
                                </button>
                            </div>
                        </section>
                    `
            ...
            ${
                canAccessContent && highlightPost
                    ? `
                            ...
                            <div class="profile-featured-layout">
                                ...
                                <div class="profile-featured-copy">
                                    ...
                                    <div class="art-stats">
                                        ...
                                        ${renderMetricText("boost", formatMetricLabel(highlightPost.boostsCount || 0, "impulso", "impulsos"))}
            ...
            <section class="section-shell" id="profilePostsSection">
                    ...
                    <div>
                        ...
                        <h2>${
                            !canAccessContent && !isOwnProfile
                                ? "Grade privada"
                                : posts.length
                                  ? `${formatCompact.format(posts.length)} publicacoes`
                ...
                <div class="art-grid">
                    ${
                        !canAccessContent && !isOwnProfile
                            ? `
                                <article class="profile-private-grid-card">
                                    <div class="card-copy">
                                        <strong>Grade reservada para seguidores.</strong>
                                        <span>Siga @${escapeHtml(profileUser.handle)} para liberar os posts e ver o ritmo completo desse perfil.</span>
                                    </div>
                                    <button
                                        class="follow-button ${isFollowed ? "is-active" : ""}"
                                        type="button"
...
function renderHero({ kicker, title, text, stats }) {
    return `
        <section class="view-hero view-hero--compact">
            <div class="view-hero-head">
                <span class="section-kicker">${escapeHtml(kicker)}</span>
                <div>
...
function renderViewTabsStrip({ group, kicker, title, text, tabs }) {
    return `
        <section class="tag-strip tag-strip--modes tag-strip--compact">
            <div class="strip-copy strip-copy--compact">
                <span class="section-kicker">${escapeHtml(kicker)}</span>
                <p>
            ...
            <div class="trend-pill-row trend-pill-row--compact">
                ${tabs
                    .map(
                        (tab) => `
                            <button
                                ...
                                aria-label="${escapeAttribute(`${tab.label}: ${formatCompact.format(tab.count || 0)}`)}"
                            >
                                <span>${escapeHtml(tab.label)}</span>
                            </button>
                        `
...
function renderDirectSubmitButton(isLoading = false) {
    return isLoading
        ? `
            <span class="button-symbol button-symbol--loader" aria-hidden="true">
...
function renderIcon(name) {
    const icons = {
        ...
        boost: `
            <svg viewBox="0 0 24 24">
                <path d="M14 5.5h3.5A1.5 1.5 0 0 1 19 7v3.5"></path>
                <path d="M18.8 5.2 12 12"></path>
                <path d="m12 7.5 0 4.5 4.5 0"></path>
                <path d="M6 18.5h8.5"></path>
            </svg>
        `,
        comment: `
            <svg viewBox="0 0 24 24">
        ...
        message: `
            <svg viewBox="0 0 24 24">
                <path d="M5 7.8A3.8 3.8 0 0 1 8.8 4h6.4A3.8 3.8 0 0 1 19 7.8v4.4A3.8 3.8 0 0 1 15.2 16H10l-4.5 3V7.8Z"></path>
                <path d="M8.5 8.5h7"></path>
                <path d="M8.5 11.5H13"></path>
            </svg>
        `,
        send: `
            <svg viewBox="0 0 24 24">
        ...
        shield: `
            <svg viewBox="0 0 24 24">
                <path d="M12 4.5 6 6.9v4.7c0 3.7 2.4 7.1 6 8.9 3.6-1.8 6-5.2 6-8.9V6.9Z"></path>
        ...
        <article class="user-card">
            ...
            <div class="user-card-body">
                ...
                <div class="user-card-stats">
                    ...
                    ${renderUserPrivacyPresencePills(user, viewerId)}
...
function renderProfilePresenceSection(profileUser, currentUser, isOwnProfile, profileTheme, highlightPost) {
    ...
    const presence = getPresenceState(profileUser, currentUser.id);
    const visibilityLabel = isProfilePrivate(profileUser) ? "Conta privada" : "Conta publica";
    const presenceScopeLabel =
        normalizePresenceVisibility(profileUser.presenceVisibility) === "everyone"
            ? "Status visivel para todos."
            : normalizePresenceVisibility(profileUser.presenceVisibility) === "followers"
              ? "Status visivel so para seguidores."
              : "Status oculto para outras pessoas.";

    return `
        <section class="profile-presence-grid">
            <article class="profile-presence-card">
                <span class="section-kicker">Privacidade</span>
                <div class="profile-presence-copy">
                    <strong>${escapeHtml(visibilityLabel)}</strong>
                    <span>${escapeHtml(presence ? `${presence.label}. ${presenceScopeLabel}` : presenceScopeLabel)}</span>
                </div>
                <div class="profile-presence-tags">
                    ${renderUserPrivacyPresencePills(profileUser, currentUser.id, { includeHiddenHint: isOwnProfile }) || `<span class="meta-pill">Artista visual</span>`}
                </div>
            </article>
            <article class="profile-presence-card">
...
function getPostActivityEntries(post) {
    ...
    return (state.db.activitiesByUser[post.author.id] || [])
        .filter((entry) => entry.postId === post.id && ["like", "boost", "comment", "reply"].includes(entry.type))
...
function createSocialPulse(post) {
    ...
    const boostCount = recentEntries.filter((entry) => entry.type === "boost").length;
    const activityCount = recentEntries.length;
    let title = "Fluxo social";
    let body = `Publicado em ${post.category} · ${timeAgo(post.createdAt)}`;

    if (boostCount >= 2) {
        title = "Ganhando alcance";
        body = `${formatMetricLabel(boostCount, "impulsionamento recente", "impulsionamentos recentes")} colocando o post em mais telas.`;
        ...
        <section class="section-shell section-shell--timeline">
            <div class="section-head">
                <div>
                    <span class="section-kicker">Timeline</span>
                    <h2>Agora na rede</h2>
                    <p class="section-note">Uma leitura direta do que esta rendendo agora, sem pesar a tela.</p>
                </div>
            </div>
...
function renderArtCard(post, options = {}) {
    ...
    const caption = truncateText(post.caption || "", options.main ? 112 : isHomeCard ? 88 : 76);
    ...
    return `
        ...
        >
            ...
            <div class="art-meta art-meta--compact">
                ${caption ? `<p class="art-caption">${escapeHtml(caption)}</p>` : ""}
                ${
                    tags.length
                        ? `
                ...
                <div class="card-actions card-actions--minimal ${isHomeCard ? "card-actions--social" : ""}">
                    ...
                    <button
                        class="icon-action icon-action--boost ${post.isBoosted ? "is-active" : ""}"
                        type="button"
                        data-action="toggle-boost"
                        data-post-id="${escapeHtml(post.id)}"
                        aria-label="${post.isBoosted ? "Remover impulso" : "Impulsionar"} ${escapeAttribute(post.title)}"
                    >
                        <span class="icon-action-symbol" aria-hidden="true">${renderIcon("boost")}</span>
                        <span class="icon-action-count">${escapeHtml(formatCompact.format(post.boostsCount || 0))}</span>
                    </button>
                    <button
                        class="icon-action"
                        type="button"
...
function renderFeedCard(post, options = {}) {
    ...
    return `
        <article
            class="feed-card ${getPostFontClass(post)} ${isInlineOpen ? "is-comments-open" : ""} ${options.highlight ? "feed-card--highlight" : ""}"
            data-card-open="true"
            data-post-id="${escapeHtml(post.id)}"
            tabindex="0"
            role="button"
        ...
        >
            ...
            <div class="feed-layout">
                ...
                <div class="feed-side">
                    ...
                    ${post.caption ? `<p class="feed-caption">${escapeHtml(truncateText(post.caption, 180))}</p>` : ""}
                    ${
                        post.tags?.length
                            ? `
                                <div class="meta-line">
...
function renderDiscussionCard(post, options = {}) {
    ...
    const replyTargetAuthor = replyTarget ? getUserById(replyTarget.authorId) : null;
    const commentPreview = renderFeedCommentPreview(post);
    const leadText = truncateText(post.caption || post.title || "Sem texto por enquanto.", options.highlight ? 220 : 160);
                ...
                entry.type === "message"
                    ? `
                        <div class="modal-actions">
                            <button class="ghost-button" type="button" data-action="open-direct-with-user" data-user-id="${escapeHtml(actor?.id || "")}">
                                ${renderButtonContent("message", "Abrir direct")}
                            </button>
                        </div>
                    `
                    : post
...
function renderDirectMessageBubble(message, viewerId, author) {
                    ...
                    <div class="modal-post-head-actions">
                        ${
                            post.author.id !== currentUser.id
                                ? `
                                    <button class="ghost-button ghost-button--compact" type="button" data-action="open-direct-with-user" data-user-id="${escapeHtml(post.author.id)}">
                                        ${renderButtonContent("message", "Mensagem")}
                                    </button>
                                    <button
                                        class="follow-button ${post.isFollowed ? "is-active" : ""}"
...
function renderComment(comment, post, depth = 0) {
    ...
    const canDelete = currentUser.id === comment.authorId || currentUser.id === post.author.id || isModerator(currentUser);
...
function renderComposerPreviewCard(draft, currentUser) {
    ...
    const previewPost = {
        ...
        caption:
            draft.caption ||
            (normalizePostKind(draft.postKind) === "discussion"
...
function getComposerModeValue(draft) {
    if (normalizePostKind(draft?.postKind) === "discussion") {
        return "discussion";
    }

    return normalizeComposerContentMode(draft?.contentMode) === "text" ? "text" : "media";
}

function renderComposerModeSwitch(draft) {
    const activeMode = getComposerModeValue(draft);

    return `
        <section class="composer-mode-switch">
            ${[
                { value: "media", label: "Imagem", note: "Arte visual" },
                { value: "text", label: "Texto", note: "Post rapido" },
                { value: "discussion", label: "Discussao", note: "Abrir conversa" }
            ]
                .map(
                    (mode) => `
                        <button
                            class="composer-mode-pill ${activeMode === mode.value ? "is-active" : ""}"
                            type="button"
                            data-action="composer-set-mode"
                            data-mode="${escapeHtml(mode.value)}"
                            aria-pressed="${activeMode === mode.value ? "true" : "false"}"
                        >
...
function renderComposerDistributionSwitch(draft) {
    const postKind = normalizePostKind(draft?.postKind);
    const distribution = normalizePostDistribution(draft?.distribution);

    if (postKind === "discussion") {
        return "";
    }

    return `
        <section class="composer-route-switch">
            ${[
                { value: "both", label: "Home + Feed" },
                { value: "feed", label: "So feed" },
                { value: "home", label: "So home" }
            ]
                .map(
                    (item) => `
                        <button
                            class="composer-route-pill ${distribution === item.value ? "is-active" : ""}"
                            type="button"
                            data-action="composer-set-distribution"
                            data-distribution="${escapeHtml(item.value)}"
                            aria-pressed="${distribution === item.value ? "true" : "false"}"
                        >
...
function isComposerAdvancedConfigured(draft) {
    const normalizedPostKind = normalizePostKind(draft?.postKind);
    const defaults = createEmptyDraft(normalizedPostKind);
    const presentation = normalizePostPresentation(draft?.presentation);
    const defaultPresentation = createDefaultPostPresentation();

    return (
        normalizeComposerContentMode(draft?.contentMode) !== defaults.contentMode ||
        normalizePostDistribution(draft?.distribution) !== defaults.distribution ||
        sanitizeText(draft?.tags || "", 90) !== "" ||
        sanitizeText(draft?.category || "", 40) !== defaults.category ||
        presentation.surfaceTone !== defaultPresentation.surfaceTone ||
        presentation.titleColor !== defaultPresentation.titleColor ||
        presentation.captionColor !== defaultPresentation.captionColor ||
        presentation.fontPreset !== defaultPresentation.fontPreset ||
        Boolean(presentation.sticker.src)
    );
}

function renderComposerAdvancedSection(draft, options = {}) {
    const presentation = draft.presentation || createDefaultPostPresentation();
    const postKind = normalizePostKind(draft.postKind);
    const distribution = normalizePostDistribution(draft.distribution);

    return `
        <details class="composer-advanced"${options.open ? " open" : ""}>
            <summary>
                <div class="composer-advanced-copy">
                    <span class="section-kicker">Opcional</span>
                    <strong>Ajustar categoria, tags e visual</strong>
                    <span>Abra so se quiser refinar o post. O fluxo principal fica leve no celular.</span>
                </div>
                <span class="composer-advanced-toggle" aria-hidden="true">${renderIcon("chevron-down")}</span>
            </summary>
            <div class="composer-advanced-body">
                <section class="customizer-panel customizer-panel--routing">
                    <div class="customizer-head">
                        <div class="card-copy">
                            ...
                            <span>Escolha como esse post entra na rede.</span>
                        </div>
                    </div>
                    <div class="customizer-grid">
                        ...
                        <label class="field field--compact">
                            ...
                            <select name="contentMode">
                                <option value="media" ${options.contentMode === "media" ? "selected" : ""}>Com imagem</option>
                                <option value="text" ${options.contentMode === "text" ? "selected" : ""}>So texto</option>
                            </select>
                        </label>
                        <label class="field field--compact">
                            ...
                            <select name="distribution">
                                ${
                                    postKind === "discussion"
                                        ? `<option value="discussion" selected>So em discussoes</option>`
                                        : `
                ...
                <div class="customizer-grid customizer-grid--meta">
                    <label class="field">
...
function refreshComposerSubmitState(form) {
    if (!form) {
        return;
    }
    ...
    const hasVisualContent = Boolean(draft.imageData);
    const canPublish =
        Boolean(postKind === "discussion" ? hasTextContent : hasTextContent || hasVisualContent) &&
        !state.ui.uploadingImage &&
        !state.ui.uploadingSticker &&
        !state.ui.publishingPost;
    const composerFootnote = state.ui.publishingPost
        ? "Publicando agora. Segura so mais um instante."
        : hasTextContent
          ? "Pronto para publicar."
          : "Escreva o titulo ou o texto.";
    const publishLabel = state.ui.publishingPost
        ? "Publicando..."
        : state.ui.uploadingImage || state.ui.uploadingSticker
          ? "Preparando..."
          : "Publicar";
    const submitButton = form.querySelector('button[type="submit"]');
    const footnote = form.querySelector(".composer-footnote");

    if (submitButton) {
        submitButton.disabled = !canPublish;
        submitButton.innerHTML = renderButtonContent("compose", publishLabel);
    }

    if (footnote) {
        footnote.textContent = composerFootnote;
    }
}
...
function renderProfileEditorPreviewContent(currentUser, draft) {
    ...
    return `
        ...
        ${previewUser.statusNote ? `<p class="profile-status-note">${escapeHtml(previewUser.statusNote)}</p>` : ""}
    `;
}
...
function renderComposerModal() {
    ...
    if (postKind === "discussion") {
        composerModalContent.innerHTML = `
            <div class="composer-layout composer-layout--discussion">
                <div class="composer-form-wrap">
                    ...
                    <form class="composer-form composer-form--discussion" id="composerForm">
                        ...
                        <input name="distribution" type="hidden" value="discussion">
                        <input name="category" type="hidden" value="Editorial">
                        <section class="composer-discussion-shell">
                            <div class="composer-discussion-head">
                                ${renderStaticProfileIdentity(currentUser, `@${currentUser.handle}`)}
                                <span class="meta-pill">Discussao</span>
                            </div>
                            <label class="field field--discussion">
                                <span>Titulo</span>
                                <input name="title" type="text" maxlength="60" placeholder="Ex.: O que voce achou disso?" value="${escapeAttribute(draft.title)}">
                            </label>
                            <label class="field field--discussion field--discussion-body">
                                <span>Texto</span>
                                <textarea name="caption" rows="7" maxlength="240" placeholder="Escreva de forma direta e puxe a conversa.">${escapeHtml(draft.caption)}</textarea>
                            </label>
                            <label class="field field--discussion">
                                <span>#</span>
                                <input name="tags" type="text" maxlength="90" placeholder="#design #cinema #opiniao" value="${escapeAttribute(draft.tags)}">
                            </label>
                        </section>
                        <div class="composer-footer composer-footer--discussion">
                            <span class="composer-footnote">${composerFootnote}</span>
                            <button class="primary-button" type="submit" ${canPublish ? "" : "disabled"}>
                                ${renderButtonContent("compose", publishLabel)}
                            </button>
                        </div>
                    </form>
                </div>
        ...
        <div class="composer-layout composer-layout--simple">
            <div class="composer-form-wrap">
                ...
                <form class="composer-form" id="composerForm">
                    ...
                    <section class="composer-quick-hint">
                        <span class="section-kicker">Rapido</span>
                        <div class="card-copy">
                            <strong>Poste em poucos toques</strong>
                            <span>Escolha a imagem, escreva a legenda e publique. Os ajustes finos ficam num bloco opcional.</span>
                        </div>
                    </section>
                    ${
                        contentMode === "text"
                    ...
                    <label class="field">
                        ...
                        <input name="title" type="text" maxlength="60" placeholder="${escapeAttribute(postKind === "discussion" ? "Ex.: O que voces acharam disso?" : "Se preferir, o app usa a legenda como titulo")}" value="${escapeAttribute(draft.title)}">
                    </label>
                    <label class="field">
                        <span>${postKind === "discussion" ? "Texto" : "Legenda"}</span>
                        <textarea name="caption" rows="5" maxlength="240" placeholder="${escapeAttribute(postKind === "discussion" ? "Solte a ideia, puxe a conversa e use # para marcar o assunto." : "Escreva como se fosse um post de verdade")}">${escapeHtml(draft.caption)}</textarea>
                    </label>
                    ${renderComposerAdvancedSection(draft, {
                        contentMode,
                        open: isComposerAdvancedConfigured(draft) || state.ui.uploadingSticker
                    })}
                    <div class="composer-submit-bar">
                        <span class="composer-footnote">${composerFootnote}</span>
                        <div class="composer-actions composer-actions--submit">
                            <button class="primary-button" type="submit" ${canPublish ? "" : "disabled"}>
                                ${renderButtonContent("compose", publishLabel)}
                            </button>
            ...
            <div class="composer-form-wrap">
                ...
                <form class="composer-form" id="profileForm">
                    ...
                    <label class="field">
                        ...
                        <input name="statusNote" type="text" maxlength="90" placeholder="O que voce esta pensando agora?" value="${escapeAttribute(draft.statusNote)}">
                    </label>
                    ...
                    <div class="profile-editor-grid">
                        <label class="field">
                            ...
                            <select name="profileVisibility">
                                <option value="public" ${draft.profileVisibility === "public" ? "selected" : ""}>Publica</option>
                                <option value="private" ${draft.profileVisibility === "private" ? "selected" : ""}>Privada</option>
                            </select>
                        </label>
                        <label class="field">
                            <span>Status</span>
                            <select name="presenceVisibility">
                                <option value="everyone" ${draft.presenceVisibility === "everyone" ? "selected" : ""}>Online e visto</option>
                                <option value="followers" ${draft.presenceVisibility === "followers" ? "selected" : ""}>So seguidores</option>
                                <option value="nobody" ${draft.presenceVisibility === "nobody" ? "selected" : ""}>Oculto</option>
                            </select>
                        </label>
                    </div>
                    <div class="file-drop file-drop--simple">
                    ...
                    <section class="customizer-panel">
                        <div class="customizer-head">
                            <div class="card-copy">
                                <strong>Senha</strong>
                                <span>Se quiser, troque sua senha por aqui sem sair da conta.</span>
                            </div>
                        </div>
                        <div class="profile-editor-grid">
                            <label class="field">
                                <span>Senha atual</span>
                                <input name="currentPassword" type="password" minlength="8" maxlength="120" placeholder="Digite sua senha atual">
                            </label>
                            <label class="field">
                                <span>Nova senha</span>
                                <input name="newPassword" type="password" minlength="8" maxlength="120" placeholder="Crie uma nova senha">
                            </label>
                        </div>
                        <label class="field">
                            <span>Confirmar nova senha</span>
                            <input name="confirmPassword" type="password" minlength="8" maxlength="120" placeholder="Repita a nova senha">
                        </label>
                    </section>
                    <div class="composer-actions">
...
function syncTopbarAlertsBadge(user) {
    ...
    const count = user ? getUnreadActivitiesCount(user.id) : 0;
    ...
    posts.forEach((post) => {
        (post.tags || [])
            ...
            .forEach((tag) => {
                ...
                current.boostsCount += post.boostsCount || 0;
                current.heat += (post.rankScores?.discussion || 0) + post.commentsCount * 6 + (post.boostsCount || 0) * 2.4 + post.likesCount * 1.8;
...
function calculateRawEngagement(postId) {
    return getLikeCount(postId) * 2.2 + getBoostCount(postId) * 4.6 + getComments(postId).length * 3.8 + getSaveCount(postId) * 3.4;
}

function getRecentPostActivityWeight(post) {
    return (state.db.activitiesByUser[post.author.id] || []).reduce((score, entry) => {
        if (entry.postId !== post.id || !["like", "boost", "comment", "reply"].includes(entry.type)) {
            return score;
        }

        const ageInHours = Math.max(0.15, (Date.now() - entry.createdAt) / 3600000);
        const interactionWeight =
            entry.type === "boost" ? 2.2 : entry.type === "reply" ? 1.4 : entry.type === "comment" ? 1.8 : 1.15;
...
function decoratePostForViewer(post, context) {
    ...
    const engagement = post.likesCount * 2.2 + post.boostsCount * 4.6 + post.commentsCount * 3.8 + post.savesCount * 3.4;
...
function getRecommendationBadge(signals) {
    if ((signals.boostsCount || 0) >= 3 && signals.ageInHours <= 24) {
        return { label: "Impulsionado", tone: "spotlight" };
    }
...
function sortPostsByConversation(posts) {
    return posts
        .slice()
        .sort((a, b) => b.commentsCount - a.commentsCount || b.boostsCount - a.boostsCount || b.likesCount - a.likesCount || b.createdAt - a.createdAt);
}

function sortPostsBySaves(posts) {
    return posts
        .slice()
        .sort((a, b) => b.savesCount - a.savesCount || b.boostsCount - a.boostsCount || b.likesCount - a.likesCount || b.createdAt - a.createdAt);
}

function sortPostsByLikes(posts) {
    return posts
        .slice()
        .sort((a, b) => b.likesCount - a.likesCount || b.boostsCount - a.boostsCount || b.savesCount - a.savesCount || b.createdAt - a.createdAt);
}
...
function calculateUserScore(user, viewerId, query = "") {
    ...
    const engagement = posts.slice(0, 6).reduce((score, post) => {
        return score + getLikeCount(post.id) * 2.4 + getBoostCount(post.id) * 4.2 + getComments(post.id).length * 3.2 + getSaveCount(post.id) * 2.1;
    }, 0);
...
function toPostView(post, viewerId) {
    ...
    if (!canViewerAccessUserContent(author, viewerId)) {
        return null;
    }
    ...
    return {
        ...
        boostsCount: getBoostCount(post.id),
        savesCount: getSaveCount(post.id),
        isOwner: post.authorId === viewerId,
        canDelete: post.authorId === viewerId || isModerator(getUserById(viewerId)),
        isLiked: getLikedIds(viewerId).includes(post.id),
        isBoosted: getBoostedIds(viewerId).includes(post.id),
...
function describeActivity(entry, actor, post) {
    switch (entry.type) {
        ...
        case "boost":
            return {
                title: "Impulsionou seu post",
                subtitle: post ? post.title : `@${actor.handle}`,
                body: "Esse post ganhou mais alcance no feed."
            };
        case "comment":
            return {
        ...
        case "message":
            return {
                title: "Mandou direct",
                subtitle: `@${actor.handle}`,
                body: entry.text || "Tem mensagem nova esperando no seu privado."
            };
        case "published":
        default:
            return {
...
function getUnreadActivitiesCount(userId) {
    const lastSeenAt = Math.max(0, Number(state.db.alertsSeenAtByUser?.[userId]) || 0);

    return getActivitiesForUser(userId).filter((entry) => entry.createdAt > lastSeenAt).length;
}

function getUnreadActivities(userId) {
    const lastSeenAt = Math.max(0, Number(state.db.alertsSeenAtByUser?.[userId]) || 0);
    return getActivitiesForUser(userId).filter((entry) => entry.createdAt > lastSeenAt);
}

function getReadActivities(userId) {
    const lastSeenAt = Math.max(0, Number(state.db.alertsSeenAtByUser?.[userId]) || 0);
    return getActivitiesForUser(userId).filter((entry) => entry.createdAt <= lastSeenAt);
}
...
function markAlertsAsSeen(userId) {
    ...
    const latestActivityAt = getActivitiesForUser(userId)[0]?.createdAt || 0;
    const nextSeenAt = Math.max(Date.now(), latestActivityAt);

    if ((state.db.alertsSeenAtByUser?.[userId] || 0) >= nextSeenAt) {
        return state.db.alertsSeenAtByUser[userId] || nextSeenAt;
    }

    state.db.alertsSeenAtByUser[userId] = nextSeenAt;
    persistDb();
    return nextSeenAt;
}
...
function isProfilePrivate(user) {
    return normalizeProfileVisibility(user?.profileVisibility) === "private";
}
...
function canViewerSeePresence(user, viewerId) {
    if (!user) {
        return false;
    }
    ...
    const visibility = normalizePresenceVisibility(user.presenceVisibility);
...
function getPresenceState(user, viewerId) {
    if (!canViewerSeePresence(user, viewerId)) {
        return null;
    }

    const isOwnViewer = user.id === viewerId;
    const online = isOwnViewer ? true : Boolean(user.isOnline);
    const lastSeenAt = Number(user.lastSeenAt) || Number(user.lastLoginAt) || Number(user.createdAt) || Date.now();

    return {
        online,
        label: online ? "Online agora" : `Visto por ultimo ${timeAgo(lastSeenAt)}`,
        tone: online ? "online" : "last-seen"
    };
}

function renderUserPrivacyPresencePills(user, viewerId, options = {}) {
    const pills = [];
    const visibility = isProfilePrivate(user) ? "Privado" : "Publico";
    pills.push(`<span class="meta-pill meta-pill--status meta-pill--${isProfilePrivate(user) ? "private" : "public"}">${escapeHtml(visibility)}</span>`);

    const presence = getPresenceState(user, viewerId);

    if (presence) {
        pills.push(`<span class="meta-pill meta-pill--status meta-pill--${escapeHtml(presence.tone)}">${escapeHtml(presence.label)}</span>`);
    } else if (options.includeHiddenHint) {
        pills.push(`<span class="meta-pill meta-pill--status meta-pill--hidden">Status oculto</span>`);
    }
...
function getBoostedIds(userId) {
    return state.db.boostsByUser[userId] || [];
}
...
function getDirectThreadsForUser(userId) {
    return (state.db.directThreads || [])
        .filter((thread) => thread.participantIds.includes(userId))
        .sort((a, b) => b.updatedAt - a.updatedAt);
}

function getActiveDirectThread(userId) {
    const threads = getDirectThreadsForUser(userId);
    ...
    const activeThread = threads.find((thread) => thread.id === state.ui.directThreadId) || threads[0];
    state.ui.directThreadId = activeThread.id;
    return activeThread;
}

function getThreadOtherUser(thread, viewerId) {
    const otherUserId = thread?.participantIds.find((userId) => userId !== viewerId);
    return otherUserId ? getUserById(otherUserId) : null;
}

function getUnreadDirectCountForThread(thread, viewerId) {
    return (thread?.messages || []).filter((message) => {
        const readBy = Array.isArray(message.readBy) ? message.readBy : [];
        return message.senderId !== viewerId && !readBy.includes(viewerId);
    }).length;
}

function getUnreadDirectThreadsCount(userId) {
    return getDirectThreadsForUser(userId).reduce((count, thread) => count + (getUnreadDirectCountForThread(thread, userId) ? 1 : 0), 0);
}
...
function getBoostCount(postId) {
    return Object.values(state.db.boostsByUser).reduce((count, boostedPosts) => {
        return count + (boostedPosts.includes(postId) ? 1 : 0);
    }, 0);
}
...
function setCollectionItemState(map, userId, itemId, isActive) {
    if (!userId || !itemId) {
        return;
    }

    const currentItems = Array.isArray(map[userId]) ? map[userId] : [];
    const withoutItem = currentItems.filter((currentId) => currentId !== itemId);
    map[userId] = isActive ? [itemId, ...withoutItem] : withoutItem;
}

function setLikeState(userId, postId, isLiked) {
    ensureUserCollections(userId);
    setCollectionItemState(state.db.likesByUser, userId, postId, isLiked);
}

function setBoostState(userId, postId, isBoosted) {
    ensureUserCollections(userId);
    setCollectionItemState(state.db.boostsByUser, userId, postId, isBoosted);
}

function setSaveState(userId, postId, isSaved) {
    ensureUserCollections(userId);
    setCollectionItemState(state.db.savesByUser, userId, postId, isSaved);
}

function setFollowState(userId, targetUserId, isFollowing) {
    ensureUserCollections(userId);
    setCollectionItemState(state.db.followsByUser, userId, targetUserId, isFollowing);
}

function upsertComment(postId, nextComment) {
    if (!postId || !nextComment?.id) {
        return;
    }

    const currentComments = Array.isArray(state.db.commentsByPost[postId]) ? state.db.commentsByPost[postId] : [];
    const withoutComment = currentComments.filter((comment) => comment.id !== nextComment.id);
    state.db.commentsByPost[postId] = [...withoutComment, nextComment].sort((a, b) => a.createdAt - b.createdAt);
}

function removeCommentsByIds(postId, deletedCommentIds) {
    if (!postId || !Array.isArray(deletedCommentIds) || !deletedCommentIds.length) {
        return;
    }

    const deletedSet = new Set(deletedCommentIds);
    const nextComments = (state.db.commentsByPost[postId] || []).filter((comment) => !deletedSet.has(comment.id));

    if (nextComments.length) {
        state.db.commentsByPost[postId] = nextComments;
        return;
    }
...
function appendDirectMessage(threadId, nextMessage) {
    if (!threadId || !nextMessage?.id) {
        return;
    }

    const thread = (state.db.directThreads || []).find((item) => item.id === threadId);

    if (!thread) {
        return;
    }

    thread.messages = [...(thread.messages || []).filter((message) => message.id !== nextMessage.id), nextMessage].slice(-400);
    thread.updatedAt = Math.max(Number(thread.updatedAt) || 0, Number(nextMessage.createdAt) || Date.now());
}

function markThreadMessagesAsRead(threadId, viewerId) {
    ...
    thread.messages = (thread.messages || []).map((message) => ({
        ...message,
        readBy: Array.isArray(message.readBy) && message.readBy.includes(viewerId) ? message.readBy : [...(message.readBy || []), viewerId]
    }));
}
...
function loginUser(userId) {
    ...
    if (previousUser && previousUser.id !== userId) {
        previousUser.lastLoginAt = Date.now();
    }

    ensureUserCollections(userId);
    state.db.alertsSeenAtByUser[userId] = Math.max(0, Number(state.db.alertsSeenAtByUser?.[userId]) || 0);
...
function ensureUserCollections(userId) {
    ...
    if (!state.db.boostsByUser[userId]) {
        state.db.boostsByUser[userId] = [];
    }
    ...
    if (!state.db.alertsSeenAtByUser[userId]) {
        state.db.alertsSeenAtByUser[userId] = 0;
    }
...
function syncComposerDraftFromForm(form) {
    ...
    draft.category = CATEGORIES.includes(String(formData.get("category") || ""))
        ...
        : requestedPostKind === "discussion"
          ? "Editorial"
          : "Ilustracao";
    if (requestedPostKind === "discussion") {
        draft.category = "Editorial";
        draft.contentMode = "text";
        draft.distribution = "discussion";
        draft.imageData = "";
        draft.fileName = "";
        presentation.sticker = createDefaultSticker();
        draft.presentation = presentation;
        return;
    }

    draft.contentMode = normalizeComposerContentMode(String(formData.get("contentMode") || draft.contentMode));
    draft.distribution = normalizePostDistribution(String(formData.get("distribution") || draft.distribution));
...
function syncProfileDraftFromForm(form) {
    ...
    draft.statusNote = sanitizeText(String(formData.get("statusNote") || ""), 90);
    draft.location = sanitizeText(String(formData.get("location") || ""), 48);
    draft.website = sanitizeText(String(formData.get("website") || ""), 80);
    draft.profileVisibility = normalizeProfileVisibility(String(formData.get("profileVisibility") || draft.profileVisibility));
    draft.presenceVisibility = normalizePresenceVisibility(String(formData.get("presenceVisibility") || draft.presenceVisibility));
...
function applyTheme(theme) {
    ...
    const normalizedSkin = normalizeSiteSkin(state.prefs.skin);

    state.prefs.theme = normalizedTheme;
    state.prefs.skin = normalizedSkin;
    document.body.dataset.theme = normalizedTheme;
    document.body.dataset.skin = normalizedSkin;
...
function normalizeSiteSkin(value) {
    return value === "space" ? "space" : "default";
}
        ...
        .map((thread) => ({
            ...
            messages: (Array.isArray(thread.messages) ? thread.messages : [])
                ...
                .map((message) => ({
                    ...
                    readBy: uniqueList(Array.isArray(message.readBy) ? message.readBy.filter((userId) => validUserIds.has(userId)) : [])
                }))
...
function getPostVisualStyleAttribute(post) {
    ...
    if (presentation.surfaceTone) {
        ...
        styleMap["--post-card-border"] = rgbaFromHex(presentation.surfaceTone, 0.28);
        styleMap["--post-card-border-strong"] = rgbaFromHex(presentation.surfaceTone, 0.42);
    }
...
async function compressImage(file, options = {}) {
    ...
    const maxInputSizeMb = clampNumber(options.maxInputSizeMb, 2, 16, 8);

    if (file.size > maxInputSizeMb * 1024 * 1024) {
        throw new Error(`Use uma imagem com ate ${maxInputSizeMb} MB.`);
    }

    if (file.type === "image/gif") {
        if (file.size > 4 * 1024 * 1024) {
            throw new Error("Use um GIF com ate 4 MB para manter o app leve.");
        }
    ...
    const maxSide = clampNumber(options.maxSide, 720, 2800, 1920);
    const quality = clampNumber(options.quality, 0.75, 0.96, 0.9);
    ...
    const outputType = resolveCompressedImageType(file.type, options.outputType);
    ...
    return outputType === "image/png" ? canvas.toDataURL(outputType) : canvas.toDataURL(outputType, quality);
}

function resolveCompressedImageType(sourceType, preferredType = "") {
    const normalizedPreferredType = String(preferredType || "").trim().toLowerCase();

    if (["image/jpeg", "image/png", "image/webp"].includes(normalizedPreferredType) && canvasSupportsMimeType(normalizedPreferredType)) {
        return normalizedPreferredType;
    }

    if (canvasSupportsMimeType("image/webp")) {
        return "image/webp";
    }

    if (sourceType === "image/png" && canvasSupportsMimeType("image/png")) {
        return "image/png";
    }
...
const canvasMimeSupportCache = new Map();

function canvasSupportsMimeType(mimeType) {
    if (canvasMimeSupportCache.has(mimeType)) {
        return canvasMimeSupportCache.get(mimeType);
    }

    const canvas = document.createElement("canvas");
    const supported = canvas.toDataURL(mimeType).startsWith(`data:${mimeType}`);
    canvasMimeSupportCache.set(mimeType, supported);
    return supported;
}
