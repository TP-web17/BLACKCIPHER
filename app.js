...
function getAvatarMarkup({ avatarDataUrl, label, initials }) {
    if (avatarDataUrl) {
        return `<img class="avatar-image" src="${escapeHtml(avatarDataUrl)}" alt="${escapeHtml(label)}">`;
    }
...
function buildConversationItemMarkup(conversation) {
    ...
    return `
        <button class="conversation-item ${isActive ? "conversation-item-active" : ""}" type="button" data-conversation-id="${escapeHtml(conversation.id)}">
    ...
    const imageHtml = message.imageDataUrl
        ? `
            <figure class="message-media">
                <img src="${escapeHtml(message.imageDataUrl)}" alt="${escapeHtml(message.imageName || "Imagem enviada")}">
            </figure>
        `
        : "";
