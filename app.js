...
function loadDb() {
    try {
        ...
        return {
            ...
            alertsSeenAtByUser: isPlainObject(parsed.alertsSeenAtByUser) ? parsed.alertsSeenAtByUser : {},
...
function bindEvents() {
    ...
    document.addEventListener("click", async (event) => {
        ...
        try {
            ...
            if (action === "mark-alerts-read") {
                await api.markAlertsAsSeen();
                renderTopbarAlertsPreview(requireCurrentUser());
                syncTopbarAlertsBadge(requireCurrentUser());
                renderView();
                return;
            }
    ...
    document.addEventListener("submit", async (event) => {
        ...
        if (directForm) {
            ...
            try {
                await api.sendDirectMessage(threadId, text);
                directForm.reset();
                renderFrame();
...
const api = {
    ...
    async toggleLike(postId) {
        const likedBefore = getLikedIds(getCurrentUser()?.id || "").includes(postId);
        await apiRequest(`/api/posts/${encodeURIComponent(postId)}/like`, {
            method: "POST"
        });
        return !likedBefore;
    },
    ...
    async toggleSave(postId) {
        const savedBefore = getSavedIds(getCurrentUser()?.id || "").includes(postId);
        await apiRequest(`/api/posts/${encodeURIComponent(postId)}/save`, {
            method: "POST"
        });
        return !savedBefore;
    },
    ...
    async toggleFollow(targetUserId) {
        const followingBefore = getFollowingIds(getCurrentUser()?.id || "").includes(targetUserId);
        await apiRequest(`/api/follows/${encodeURIComponent(targetUserId)}`, {
            method: "POST"
        });
    ...
    async addComment(postId, rawText, replyToCommentId = "") {
        await apiRequest(`/api/posts/${encodeURIComponent(postId)}/comments`, {
            method: "POST",
            body: {
                text: rawText,
                replyToCommentId
    ...
    async deleteComment(postId, commentId) {
        await apiRequest(`/api/posts/${encodeURIComponent(postId)}/comments/${encodeURIComponent(commentId)}`, {
            method: "DELETE"
        });
        return [commentId];
    },
    ...
    async markAlertsAsSeen() {
        await apiRequest("/api/alerts/seen", {
            method: "POST"
        });
    },
    ...
    async sendDirectMessage(threadId, text) {
        await apiRequest(`/api/directs/${encodeURIComponent(threadId)}/messages`, {
            method: "POST",
            body: { text }
        });
        return true;
